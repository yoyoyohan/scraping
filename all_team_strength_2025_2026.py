...
DATA_DIR = Path("data")
OUTPUT_PATH = DATA_DIR / f"all_team_strength_{SEASON.replace('-', '_')}.csv"
...
def main() -> None:
    ...
    team_urls = get_all_team_urls()  # all conferences
    ...
    if not team_games:
        print(f"No schedules found for season {SEASON}.")
        return
    ...
    print(f"\nSaved statewide team strength rankings to {OUTPUT_PATH}")
